...
const { AuthenticationError, BadRequest, DDoSProtection, ExchangeError, ExchangeNotAvailable, InsufficientFunds, InvalidOrder, OrderNotFound, PermissionDenied, ArgumentsRequired, BadSymbol } = ccxt;
class bitmex extends ccxt.pro.bitmex {
    ...
    async fetchMarkets(params = {}) {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            // hack to add non active markets, so fetchTickers does not fail
            if (id[0] == '.') {
                symbol = id;
            }
            ...
            result.push({
                ...
                'precision': {
                    ...
                    'amount': this.safeNumber(market, 'lotSize') / this.safeNumber(market, 'underlyingToPositionMultiplier'),
    ...
    parseTicker(ticker, market = undefined) {
        ...
        return this.safeTicker({
            ...
            'average': undefined,
            'baseVolume': this.numberToString(this.convertFromRawQuantity(symbol, this.safeString(ticker, 'homeNotional24h'))),
            'quoteVolume': this.numberToString(this.convertFromRawQuantity(symbol, this.safeString(ticker, 'foreignNotional24h'))),
            'info': ticker,
        }, market);
    }
    ...
    parseTrade(trade, market = undefined) {
        ...
        const execCost = this.convertFromRawCost(symbol, this.safeString(trade, 'execCost'));
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        const qty = this.parseToInt(this.convertFromRealAmount(market['base'], amount));
        const request = {
            'symbol': market['id'],
